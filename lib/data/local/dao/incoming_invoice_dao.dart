...
class IncomingInvoiceDao {
  Future<Database> get _db async => AppDatabase.instance;

  Future<List<IncomingInvoiceEntity>> getInvoices({String search = ''}) async {
    final db = await _db;
    ...
    final result = <IncomingInvoiceEntity>[];
    for (final row in rows) {
      final items = await db.query('incoming_invoice_items', where: 'invoice_id = ?', whereArgs: [row['id']], orderBy: 'id ASC');
      result.add(
        IncomingInvoiceEntity(
          ...
          items: items.map((i) => IncomingInvoiceItemEntity(
            id: i['id'] as int?,
  ...
  Future<void> saveInvoice(IncomingInvoiceEntity invoice) async {
    ...
    await db.transaction((txn) async {
      ...
      int invoiceId;
      if (invoice.id == null) {
        invoiceId = await txn.insert('incoming_invoices', payload);
      } else {
        invoiceId = invoice.id!;
        await txn.update('incoming_invoices', payload, where: 'id = ?', whereArgs: [invoiceId]);
        await txn.delete('incoming_invoice_items', where: 'invoice_id = ?', whereArgs: [invoiceId]);
      }
      for (final item in invoice.items) {
        await txn.insert('incoming_invoice_items', {
  ...
  Future<void> deleteInvoice(int id) async {
    final db = await _db;
    await db.delete('incoming_invoices', where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<void> registerPayment({
    ...
    final db = await _db;
    await db.transaction((txn) async {
      final rows = await txn.query('incoming_invoices', where: 'id = ?', whereArgs: [invoiceId], limit: 1);
      ...
      await txn.update('incoming_invoices', {
        'paid_amount': newPaid,
        'status': status.name,
        'updated_at': DateTime.now().toIso8601String(),
      ...
      if (method == PaymentMethod.cash) {
        await txn.insert('cash_transactions', {
          'reference_no': 'PAY-IN-$invoiceId',
          'direction': 'outgoing',
          'amount': amount,
          'description': 'Incoming invoice payment #$invoiceId',

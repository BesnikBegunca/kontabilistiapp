...
import '../providers/supplier_summaries_controller.dart';

class SupplierBalancesPage extends ConsumerStatefulWidget {
  const SupplierBalancesPage({super.key});

  @override
  ConsumerState<SupplierBalancesPage> createState() => _PageState();
}

class _PageState extends ConsumerState<SupplierBalancesPage> {
  final _searchCtrl = TextEditingController();
  final _csv = CsvExportService();

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(supplierSummariesControllerProvider);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const PageHeader(title: 'Supplier Balances', subtitle: 'Track supplier liabilities, paid totals and remaining payables.'),
          ...
          children: [
            SizedBox(
              ...
              child: TextField(
                ...
                onChanged: (_) => ref.read(supplierSummarySearchProvider.notifier).state = _searchCtrl.text.trim(),
  ...
  Future<void> _export(List<dynamic> rows) async {
    final path = await _csv.export(
      suggestedName: 'supplier_balances.csv',
